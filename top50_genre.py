...
def movies_by_genres():
    ...
    print(f"\033[7;1mHow many movies do you want?\033[0m ", end='\n\n')
    how_many = int(input("Enter a number between 1 - 50: "))
    movie_idx = 1
    for i in range(how_many):
        year = top50_by_genre[i]['year']
        print(f"{movie_idx}) {top50_by_genre[i]} - {year}")
        movie_idx += 1
    print(f"\033[7;1mWhich movie's details you want to look for from the list above?\033[0m", end='\n\n')
