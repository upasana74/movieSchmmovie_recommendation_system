...
def movies_by_genres():
    ...
    how_many = int(input("How many movies you want? Enter a number between 1 - 50: "))
    movie_idx = 1
    for i in range(how_many):
        year = top50_by_genre[i]['year']
        print(f"{movie_idx}) {top50_by_genre[i]} - {year}")
        movie_idx += 1
    user_selection = int(input(f"Which movie's details you want to look for from the list above? Enter an integer 1-{how_many}: "))
